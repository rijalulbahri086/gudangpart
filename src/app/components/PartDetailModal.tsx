...
import { X, MapPin, Tag, Cpu, AlertTriangle, Package, Image as ImageIcon, ShieldCheck, Maximize2 } from 'lucide-react';
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Gambar Produk (Gaya asli, memenuhi kartu + fitur klik perbesar) */}
          <div className="relative mb-6 h-64 w-full overflow-hidden rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center group">
            {item.image_url ? (
              <>
                <img
                  src={item.image_url}
                ...
                <button
                  type="button"
                  onClick={() => setIsImagePreviewOpen(true)}
                  className="absolute bottom-3 right-3 bg-black/70 hover:bg-black text-white text-xs font-semibold px-2.5 py-1.5 rounded-lg backdrop-blur-sm flex items-center gap-1.5 transition opacity-0 group-hover:opacity-100 shadow-md"
                >
      ...
      {/* MODAL PREVIEW FULL FOTO (Full Screen) */}
      {isImagePreviewOpen && item.image_url && (
        <div 
